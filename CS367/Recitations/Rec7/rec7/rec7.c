...
extern long     bitLength (long);
extern long       sumOdds (long);
...
int main(int argc, char ** argv){
  ...
  if ( ! strncmp("clamp", funcName, 10)) { 
    check_func_params(progName, funcName, argc, 3);
    long x = arg[1], a = arg[2], b = arg[3];
    printf("%ld\n", clamp(x, a, b));
  } 
  
  else if ( ! strncmp("bitLength", funcName, 10)) {
    check_func_params(progName, funcName, argc, 1);
    long n = arg[1];
    printf("%ld\n", bitLength(n));
  }
  
  else if ( ! strncmp("sumOdds", funcName, 10)){
    check_func_params(progName, funcName, argc, 1);
    long n = arg[1];
    printf("%ld\n",sumOdds(n));
  }
